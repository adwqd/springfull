    ...
    const navigate = useNavigate();
        ...
        useEffect(() => {
            ...
            const fetchPosts = async (page, sort, keyword = "") => {
                setLoading(true);
                try {
                    const response = await axios.post(`${apiURL}/member/mypost`, {
                        page,
                        size: postsPerPage,
                            ...
                            <li key={post.id} className="p-3 border rounded-lg flex items-center justify-between transition hover:shadow-md">
                                ...
                                <div className="text-gray-500 text-sm flex flex-col items-end space-y-1 w-20">
                                    <div className="flex items-center space-x-1 w-full justify-end">
                                        <FaStar className="text-yellow-500" /> <span className="font-medium text-right w-8">{post.star ? Number(post.star).toFixed(1) : "0.0"}</span>
                                    </div>
                                    <div className="flex items-center space-x-1 w-full justify-end">
                                        <FaHeart className="text-red-500" /> <span className="font-medium text-right w-8">{post.post_like}</span>
                                    </div>
                                </div>
                            </li>
                        ))}
